...
class StatsViewModelTest {
    ...
    @Test
    fun `empty habits exposes empty stats`() =
        runTest {
    ...
    @Test
    fun `stats match manual calculation on known data`() =
        runTest {
            // 已知数据设计（可人工复算）：
    ...
    @Test
    fun `repository error exposes error message`() =
        runTest {
    ...
    @Test
    fun `heatmap aggregates completion counts across habits`() =
        runTest {
            val today = LocalDate.now()
