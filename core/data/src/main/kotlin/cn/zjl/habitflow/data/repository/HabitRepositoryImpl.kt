...
class HabitRepositoryImpl @Inject constructor(
    ...
    override fun observeHabits(): Flow<List<Habit>> =
        habitDao.observeActiveHabits().map { list -> list.map { it.toModel() } }

    override fun observeChecked(habitId: Long, date: LocalDate): Flow<Boolean> =
        habitDao.observeChecked(habitId, date.toEpochDay())

    override fun observeCompletionStats(habitId: Long, days: Int): Flow<List<Pair<LocalDate, Boolean>>> {
        val today = LocalDate.now()
        return habitDao.observeRecordsBetween(
            habitId = habitId,
    ...
    override suspend fun checkOut(habitId: Long, date: LocalDate) =
