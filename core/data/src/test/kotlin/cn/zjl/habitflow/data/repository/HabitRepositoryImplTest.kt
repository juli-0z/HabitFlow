...
class HabitRepositoryImplTest {
    ...
    @Test
    fun `observeHabits maps entities to models with all fields`() = runTest {
        val e = entity(
            id = 7,
            name = "阅读",
    ...
    @Test
    fun `observeChecked converts date to epochDay before dao call`() = runTest {
    ...
    @Test
    fun `observeCompletionStats builds full day window oldest to newest`() = runTest {
        val today = LocalDate.now()
        val todayEpoch = today.toEpochDay()
        val threeDaysAgoEpoch = today.minusDays(3).toEpochDay()
        coEvery { dao.observeRecordsBetween(1L, any(), any()) } returns flowOf(
            listOf(
    ...
    @Test
    fun `saveHabit maps model to entity and upserts`() = runTest {
        val habit = model(
            id = 9,
            name = "晨跑",
    ...
    @Test
    fun `archiveHabit delegates to dao`() = runTest {
    ...
    @Test
    fun `checkIn inserts record with epochDay date and timestamp`() = runTest {
        val date = LocalDate.of(2026, 8, 21)
    ...
    @Test
    fun `checkOut deletes record for date`() = runTest {
        val date = LocalDate.of(2026, 8, 21)
