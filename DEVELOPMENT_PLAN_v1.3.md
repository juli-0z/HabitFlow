...
- [ ] 3.1 习惯编辑页完整化：图标/颜色选择、频率（每日/每周 N 次）、目标量【必做】— 验收：编辑后数据正确落库、列表刷新（**实现+单测完成 2026-08-21，模拟器验收待复核**）
- [ ] 3.2 习惯删除：长按 → 确认对话框 → 软删除（isArchived）+ 记录保留【必做】— 验收：删除后列表消失、统计仍计入历史（**实现+单测完成 2026-08-21，模拟器验收待复核；"统计计入历史"的数据层保留已保证，界面呈现随 3.4 统计页验证**）
...
- [ ] 3.9 Repository 映射逻辑单测：`HabitRepositoryImplTest`（mock DAO，测 entity→model 映射与 Flow 组装）【必做】— 验收：`:core:data:test` 全绿（**可第 3 周随时穿插，无依赖**）
